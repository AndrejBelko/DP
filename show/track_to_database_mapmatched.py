...
try:
    dx = pd.read_csv("/home/data/import/files/original/" + dbName + "_path.csv")
    id = int(dx['track'].max()) + 1
except FileNotFoundError:
    dx = pd.DataFrame(geohash, columns=["geohash", "track"])
    dx.to_csv("/home/data/import/files/original/" + dbName + "_path.csv", index=False, mode="w")
    id = 0
...
for track in df.values:
    ...
    if row > 1000000:
        dx = pd.DataFrame(geohash, columns=["geohash", "track"])
        dx.to_csv("/home/data/import/files/original/" + dbName + "_path.csv", header=False, index=False, mode="a")
        row = 0
        geohash = []
        total += 1
...
file_path = f"/home/data/import/files/original/{dbName}_track.csv"
try:
...
for id, values in grouped:
    tracks.append(
        [id, str(geojson.Feature(geometry=geojson.LineString(values[["longitude", "latitude"]].values.tolist())))])
    row += 1
    if row > 10000:
        dx = pd.DataFrame(tracks, columns=['route', 'track'])
        dx.to_csv("/home/data/import/files/original/" + dbName + "_track.csv", header=False, index=False, sep=';',
...
try:
    mycursor.execute(f"""
        LOAD DATA LOCAL INFILE '/home/data/import/files/mapmatched/{dbName}_path.csv'
        INTO TABLE `path`
...
try:
    mycursor.execute(f"""
        LOAD DATA LOCAL INFILE '/home/data/import/files/mapmatched/{dbName}_track.csv'
        INTO TABLE `tracks`

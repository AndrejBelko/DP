...
var gdata;
var geoResult = null, geoAllResult = null;
var distance = 0;
...
const bsOffcanvasRight = bootstrap.Offcanvas.getOrCreateInstance(sidebarRight);

let opened = false;

L.easyButton('fa-list', function () {
    bsOffcanvas.toggle();
    changeSideBar()
}).addTo(map);

graphButton = L.easyButton('fa-bar-chart', function () {
...
function changeSideBar(){
    if (opened){
        ...
        document.querySelector('.leaflet-control-container').classList.add('sidebar-open');
        opened = true;
    }
}
...
function showSelectedColumn(point) {
    ...
    for (i = 0; i < chart2.options.data[0].dataPoints.length; i++) {
...
function showSelectedColumnPerPoint(point) {
    ...
    for (const j in fieldsTickedPerPoint) {
        for (i in gdata) {
            for (const k in gdata[i][5]) {
...
function addToPath(latlng) {
    ...
    const rect = L.rectangle([[box['latitude'][0], box['longitude'][0]], [box['latitude'][1], box['longitude'][1]]], {
        color: "#eb3a05",
        weight: 2,
        fillOpacity: 0
    });
...
function showResults() {
    ...
    const $table = $('<table class="result-table display"></table>'); // Add "display" class for DataTable styling
    const $thead = $('<thead><tr><th>Map</th><th>Details</th></tr></thead>'); // Table header
    const $tbody = $('<tbody></tbody>');

    gdata.forEach(function (item, index) {
        const dateObj = new Date(item[7]);
        ...
        const $tr = $('<tr class="result-item"></tr>').attr('id', item[0]);

        const $td1 = $('<td></td>');
        const $div1 = $('<div id="mapArea' + index + '" style="width:150px;height:80px;"></div>').on('click', function () {
            showTrack(index);
        });
        $td1.append($div1);

        const $td2 = $('<td></td>');
        const $div2 = $('<div></div>').on('click', function () {
            showTrack(index);
        });
        const $h6 = $('<h6><b>' + formattedDate + '</b><br>Path starts on box <b>' + item[2] + '</b> and ends on box <b>' + item[3] + '</b><br>In total, <b>' + item[1] + '</b> fields matched, with <b>' + item[4] + '</b> gaps<br>Matched fields: <b>' + item[5] + '</b></h6>');
        $div2.append($h6);
        $td2.append($div2);
...
if (queryDB !== " "){
    $.getJSON("coverage/"+dbname+".geojson", function (data) {
        L.geoJSON(data, {
            style: function () {
                return {
...
function showResults1() {
    ...
    const $table = $('<table class="result-table display"></table>'); // Add "display" class for DataTables styling
    const $thead = $('<thead><tr><th>Map</th><th>Details</th></tr></thead>'); // Define the table header
    const $tbody = $('<tbody></tbody>');
